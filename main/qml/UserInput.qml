...
Item {
    ...
    function userLogin() {
        ...
        if (currentUser.logined) {
            console.log("User has logined", currentUser.name)
            // Panel.proxy.unlock(currentUser.name, passwordField.text)
            // return
        }

        Panel.proxy.login(currentUser.name, passwordField.text)
    }
        ...
        D.RoundButton {
            ...
            HintLabel {
                ...
                hintText: {
                    let userModel = Panel.proxy.userModel
                    let currentUser = userModel.get(userModel.currentUserName)
                    return currentUser.passwordHint
                }
            }
